...
void addInstructions(int path[], int altchk) {
...
void makeNewGraph(int carloc[2], int graph[][V], int path[]) {
	...
	if (interruptatnode == 1) {      //when there is an interruption AT a node, not in bewteen nodes
		for (int j = 0; j < V; j++) {
			...
			if (j != prevnode[whatinterruptnode]) {
...
char Sendinstruction(int location[], int location2[], int path[]) { // location : car's location, location2 : interruption's location.
	...
	if (distance(location[0], location[1], temp.pos_x, temp.pos_y) < ROTATIONDISTANCE) {
		...
		//if (temp.pos_x == tempnext.pos_x) {
		//if ((temp.pos_y - location[1])*(tempnext.pos_y - location[1]) < 0) {

		sendinstruction = 'F';
		//}

		//}
		//else if (temp.pos_y == tempnext.pos_y) {
		//   printf("check : %d(%d-%d) x %d(%d-%d) = %d\n", temp.pos_x - location[0], temp.pos_x, location[0], tempnext.pos_x - location[0],tempnext.pos_x,location[0],(temp.pos_x - location[0])*(tempnext.pos_x - location[0]));
		//   if ((temp.pos_x - location[0])*(tempnext.pos_x - location[0]) < 0) {
...
void dijkstra(int editgraph[][V], int* editpath, int* editlength, int altchk) { // chkalt : altgraph에 의한 dijkstra인지 chk. chk ==1 이면 altgraph
	...
	*editlength = j;
	...
	addInstructions(editpath, altchk); // altchk에 따라서 instructionset or altinstructionset에 insertQ 실행
}
